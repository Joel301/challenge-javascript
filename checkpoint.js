...
function direcciones(laberinto) {
  ...
  arrPc = ["N", "S", "E", "O"];
  for (var pc in arrPc) {
    if (laberinto[arrPc[pc]] == "pared") {
      continue;
    }
    if (laberinto[arrPc[pc]] == "destino") return arrPc[pc];
    if (typeof laberinto[arrPc[pc]] === "object") {
      res = direcciones(laberinto[arrPc[pc]]);
      if (!res) continue;
      return arrPc[pc] + res;
    }
  }
...
function deepEqualArrays(arr1, arr2) {
  ...
  for (i in arr1) {
    if (typeof arr1[i] == "object" && typeof arr2[i] == "object") {
      r = deepEqualArrays(arr1[i], arr2[i]);
      if (!r) return false;
...
function multiCallbacks(cbs1, cbs2) {
  ...
  while (arr.length) {
    ...
    for (let i = 0; i < arr.length; i++) {
      if (!elemento || arr[i].time < elemento.time) elemento = arr[i];
    }
    resultado.push(elemento.cb());
...
BinarySearchTree.prototype.toArray = function () {
  var array = [];
  var f = (a) => array.push(a);
  this.depthFirstForEach(f);
  return array;
};
